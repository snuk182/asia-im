...
public class EncryptedMessage implements PacketExtension {
	...
	private String myKeyPwd;
	
	public EncryptedMessage(String keyPath, String keyPwd) {
		this.myKey = keyPath;
		this.myKeyPwd = keyPwd;
	}
	...
	public String decryptAndGet() throws Exception {
		InputStream in = new ByteArrayInputStream(encryptedText.getBytes());
		InputStream keyIn = new BufferedInputStream(new FileInputStream(myKey));
		String out = decrypt(in, keyIn, myKeyPwd.toCharArray());
		keyIn.close();
		in.close();
	...
	public void setAndEncrypt(String message, String buddyKey) throws Exception {
		...
		PGPPublicKey encKey = PGPExampleUtil.readPublicKey(buddyKey);
	...
	static byte[] compressFile(byte[] data, int algorithm) throws IOException
    {
        ...
        PGPUtil.writeToLiteralData(new ByteArrayInputStream(data), bOut, PGPLiteralData.BINARY, "");
