...
import ua.snuk182.asia.services.icq.inner.dataentity.ICQDCInfo;
import ua.snuk182.asia.services.icq.inner.dataentity.ICQIconData;
	...
	public List<ICQOnlineInfo> parseOnlineInfo(Snac snac, ICQOnlineInfo info) throws ICQException{
		...
		while (pos<snac.plainData.length){
			...
			if (!uin.equals(service.getUn())){
				nfo.visibility = service.getBuddyList().denyList.get(uin) !=null ? ICQConstants.VIS_DENIED : (service.getBuddyList().permitList.get(uin) !=null ? ICQConstants.VIS_PERMITTED : 0);
