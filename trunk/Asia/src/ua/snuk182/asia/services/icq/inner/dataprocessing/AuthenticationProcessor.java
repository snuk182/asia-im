...
public class AuthenticationProcessor extends AbstractFlapProcessor {
	...
	protected void internalTLVMap(TLV tlv){
		...
		case ICQConstants.TLV_PERSONALTEXT:
			/*if (service.getOnlineInfo()!=null){
				service.getOnlineInfo().setPersonalText(ProtocolUtils.bytes2ShortBE(tlv.value));
				service.log("    personal text: "+service.getOnlineInfo().getPersonalText());
			}*/
			break;
	...
	protected void internalSnacMap(Snac snac) throws ICQException{
		...
		case ICQConstants.SNAC_FAMILY_GENERIC:
			...
			case ICQConstants.SNAC_GENERIC_RATELIMITINFORES:
				...
				Flap[] flaps = new Flap[]{
						...
						sendRateLimitsAck(),
						requestOnlineInfo(),
						requestSSIServiceLimits(),
						requestLocationServiceLimits(),
						requestBuddylistServiceLimits(),
						requestMessagingServiceLimits(),
		...
		case ICQConstants.SNAC_FAMILY_SERVERSIDEINFO:
			...
			case ICQConstants.SNAC_SERVERSIDEINFO_LOCALSSIUPTODATE:
				Flap[] flaps = new Flap[]{sendCapabilities(),
						sendICBMParameters(),
						sendSSIActivate()};
	...
	private Flap sendICBMParameters() throws ICQException{
		...
		byte[] plainData = new byte[]{0, 0, 0, 0, 0, 0xb,0x1f, 0x40, 3, (byte) 0xe7, 3, (byte) 0xe7, 0, 0, 0, 0};// TODO fix!
	...
	private Flap sendCapabilities() throws ICQException{
		...
		pos+=16;
		/*System.arraycopy(new String("Asia 0.5.0.4.001").getBytes(), 0, rawData, pos, 16);
		pos+=16;*/
