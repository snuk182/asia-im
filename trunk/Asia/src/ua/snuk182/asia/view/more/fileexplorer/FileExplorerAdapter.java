...
public class FileExplorerAdapter extends BaseAdapter {
	...
	public FileExplorerAdapter(EntryPoint entryPoint, List<File> files){
		this.files = files;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            TextView temp = new TextView(entryPoint);
	...
	private Drawable getDrawableByFileType(File file){
		if (file.isDirectory()){
			return entryPoint.getResources().getDrawable(R.drawable.file_folder);
		} else if (file.getName().endsWith(".mp3") 
				|| file.getName().endsWith(".flac")
				|| file.getName().endsWith(".ogg")
				|| file.getName().endsWith(".ape")
				|| file.getName().endsWith(".m4a")){
			return entryPoint.getResources().getDrawable(R.drawable.file_audio);
		} else if (file.getName().endsWith(".avi")
				|| file.getName().endsWith(".mp4")
				|| file.getName().endsWith(".3gp")
				|| file.getName().endsWith(".flv")){
			return entryPoint.getResources().getDrawable(R.drawable.file_video);
		} else if (file.getName().endsWith(".txt")
				|| file.getName().endsWith(".fb2")
				|| file.getName().endsWith(".doc")
				|| file.getName().endsWith(".docx")){
			return entryPoint.getResources().getDrawable(R.drawable.file_text);
		} else if (file.getName().endsWith(".jpg")
				|| file.getName().endsWith(".jpeg")
				|| file.getName().endsWith(".gif")
				|| file.getName().endsWith(".png")
				|| file.getName().endsWith(".bmp")){
			return entryPoint.getResources().getDrawable(R.drawable.file_pict);
		} else if (file.getName().equals("")){
			return entryPoint.getResources().getDrawable(R.drawable.file_back);
		} else {
			return entryPoint.getResources().getDrawable(R.drawable.file_plain);
		}
	}
}
