...
public class SSIProcessor {
	...
	public void addGroup(ICQBuddyGroup group) {
		...
		masterGroup.buddies.addAll(service.getBuddyList().buddyGroupList);
	...
	public void parseSSIResponse(byte[] plainData) {
		...
		case 0x0:
			...
			case ACTION_ADD_GROUP:
				service.getBuddyList().buddyGroupList.add(tmpGroup.groupId);
	...
	public void sendAuthorizationRequest(ICQBuddy buddy, String reason) {
		service.getRunnableService().sendMultipleToSocket(new Flap[] { getGrantFutureAuthorizationFlap(buddy.uin), getAuthRequestFlap(buddy, reason) });
	}
	...
	private Flap getAuthRequestFlap(ICQBuddy buddy, String reason) {
		Flap flap1 = new Flap();
		flap1.channel = ICQConstants.FLAP_CHANNELL_DATA;
