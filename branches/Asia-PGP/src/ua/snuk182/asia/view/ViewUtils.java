...
public final class ViewUtils {
	...
	private static final String getBuddyPGPKey(Context context, Buddy buddy){
		Set<String> keys = new HashSet<String>();
		keys.add(buddy.protocolUid);
		return new ServiceStoredPreferences(context).getMap(keys, buddy.getOwnerAccountId() + " " + AccountService.BUDDY_KEY_STORAGE).get(buddy.protocolUid);
	}
	...
	public static void contactMenu(final EntryPoint entryPoint, final AccountView account, final Buddy buddy) {
		...
		for (int i = 0; i < items.length(); i++) {
			...
			if (item.equals(res.getString(R.string.menu_value_add_pgp_key))){
				if (!accountSupportsSecureChat(entryPoint, account)){
					continue;
				}
				
				if (getBuddyPGPKey(entryPoint, buddy) != null){
					continue;
				}				
			}
			
			if (item.equals(res.getString(R.string.menu_value_remove_pgp_key))){
				if (!accountSupportsSecureChat(entryPoint, account)){
					continue;
				}
		...
		builder.setItems(itemArray, new OnClickListener() {
			...
			public void onClick(final DialogInterface bdialog, int which) {
				...
				if (command.equals(res.getString(R.string.menu_value_add_pgp_key))){
					addPGPKey(entryPoint, buddy);
				}
				
				if (command.equals(res.getString(R.string.menu_value_remove_pgp_key))){
					removePGPKey(entryPoint, buddy);
				}
	...
	private static void removePGPKey(final EntryPoint entryPoint, final Buddy buddy) {
		Set<String> pgpSet = new HashSet<String>();
		pgpSet.add(buddy.protocolUid);
		new ServiceStoredPreferences(entryPoint).removeInMap(pgpSet, buddy.getOwnerAccountId() + " " + AccountService.BUDDY_KEY_STORAGE);		
	}

	private static void addPGPKey(final EntryPoint entryPoint, final Buddy buddy) {
		FileExplorerAction action = new FileExplorerAction() {
			
			@Override
			public void action(File file) {
				Map<String, String> pgpMap = new HashMap<String, String>();
				pgpMap.put(buddy.protocolUid, file.getAbsolutePath());
				new ServiceStoredPreferences(entryPoint).saveMap(pgpMap, buddy.getOwnerAccountId() + " " + AccountService.BUDDY_KEY_STORAGE);
			}
		};
		
		FileFilter pgpTypesFilter = new FileFilter() {
			
			@Override
			public boolean accept(File file) {
				return file.isDirectory() || file.getAbsolutePath().endsWith(".asc") || file.getAbsolutePath().endsWith(".bpg");
			}
		};
		
		ViewUtils.showPickFileDialog(entryPoint, buddy, action, pgpTypesFilter);
	}
	...
	private static void showMoveBuddiesDialog(final AccountView account, final BuddyGroup group, final List<Buddy> buddies, final EntryPoint entryPoint) {
		...
		adapter = new ArrayAdapter<BuddyGroup>(entryPoint, android.R.layout.simple_spinner_item, groups);
	...
	public static final boolean accountSupportsSecureChat(Context context, AccountView account) {
		return Boolean.parseBoolean(account.options.getString(context.getString(R.string.key_enable_pgp)));
	}
	...
	public static String[] getProxyTypes(AccountService aps, Context context) {
		...
		if (aps.getServiceName().equals(context.getResources().getString(R.string.icq_service_name))) {
			id = ICQService.PROXY_TYPE_ID;
		}
		if (aps.getServiceName().equals(context.getResources().getString(R.string.xmpp_service_name))) {
			id = XMPPService.PROXY_TYPE_ID;
		}
		if (aps.getServiceName().equals(context.getResources().getString(R.string.mrim_service_name))) {
			id = MrimService.PROXY_TYPE_ID;
		}
